...
public abstract class Database {
    ...
    public static Database select() {
        String type = UserLogin.getPlugin().getConfig().getString("database.type", "yaml");
    ...
    public boolean comparePasswords(UUID uuid, String otherPassword) {
        ...
        if (rawPassword == null)
            throw new IllegalArgumentException("UUID " + uuid + " does not have an associated password");
        ...
        if (rawPassword.startsWith("§")) {
            ...
            try {
                ...
                ObjectInputStream objInput = new ObjectInputStream(arrayInput);
                decrypted = (String) objInput.readObject();
    ...
    public boolean shouldLogConnected() {
        return logConnected;
    }
