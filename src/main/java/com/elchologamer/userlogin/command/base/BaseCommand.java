...
public abstract class BaseCommand extends Command {
    ...
    public final boolean execute(CommandSender sender, String label, String[] args) {
        ...
        if (!plugin.getConfig().getBoolean("enabledCommands." + getName(), true)) {
            sender.sendMessage(plugin.getLang().getMessage("commands.disabled"));
            return true;
        }

        if (playerOnly && !(sender instanceof Player)) {
            sender.sendMessage(plugin.getLang().getMessage("commands.errors.player_only"));
            return true;
        }
    ...
    public void register() {
        String usage = plugin.getLang().getMessage("commands.usages." + getName());
        if (usage != null) setUsage(usage);

        String description = plugin.getLang().getMessage("commands.descriptions." + getName());
